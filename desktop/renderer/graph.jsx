...
function GraphView({ graph, onOpen, bases, kbFilter, onToggleBase, baseOf, hideTitle, colorBy }) {
  ...
  React.useEffect(() => {
    ...
    for (const n of nodes) {
      ...
      else if (hadPrev) { n._born = nowTs; newborn++; }
    }
    ...
    for (const l of links) {
      ...
      if (!s || !t2) continue;
      if (s._born && !t2._born && t2.x != null) { s.x = t2.x + (Math.random() * 26 - 13); s.y = t2.y + (Math.random() * 26 - 13); }
      else if (t2._born && !s._born && s.x != null) { t2.x = s.x + (Math.random() * 26 - 13); t2.y = s.y + (Math.random() * 26 - 13); }
    }
    ...
    const render = () => {
      ...
      for (const l of ls) {
        ...
        if (l._born) {
          ...
          if (p >= 1) delete l._born;
          else if (p > 0) { dashProgress = p; animating = true; }
          else { animating = true; continue; } // scheduled but not started yet
        }
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 45, background: 'var(--surface-canvas)', display: 'flex', flexDirection: 'column', animation: 'lore-fade-in 160ms ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--text-faint)', marginTop: 1 }}>Colors match the section chips. Double-click a dot to open it.</div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      {empty ? (
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
          {/* Section coloring (GraphView default): nodes match the section-chip
              colors, so 300 same-place notes don't all render one amber blob
              ("nodes not color coded based on topic", 2026-07-09). */}
          <GraphView graph={graph} onOpen={onOpen} bases={bases} kbFilter={kbFilter} onToggleBase={onToggleBase} baseOf={baseOf} hideTitle />
        </div>
      )}
    </div>
  );
