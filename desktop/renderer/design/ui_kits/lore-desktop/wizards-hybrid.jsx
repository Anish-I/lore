    ...
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--text-faint)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{meta}</div>
        </div>
        {teamBadge && (
          <span style={{ flexShrink: 0, fontSize: 10.5, fontWeight: 600, color: 'var(--place-team-fg)', background: 'var(--place-team-tint)', border: '1px solid var(--place-team-border)', borderRadius: 999, padding: '2px 9px' }}>Team</span>
        )}
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        {onChat && <WzButton variant="amber-ghost" icon="sparkles" onClick={onChat} style={{ flex: 1 }}>Chat with it</WzButton>}
        {extraAction}
      </div>
    </div>
  );
...
function CreateTile({ onClick }) {
  ...
  return (
    <button onClick={onClick}
      ...
      <WzIcon name="plus" size={20} />
      <span style={{ fontSize: 13, fontWeight: 600 }}>Create one</span>
      <span style={{ fontSize: 11.5, color: 'var(--text-faint)' }}>e.g. “everything about Northwind”</span>
    </button>
  );
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderBottom: '1px solid var(--divider)', flexShrink: 0 }}>
          <span style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'var(--brand-soft-bg)', border: '1px solid var(--brand-soft-border)' }}>
            <WzIcon name="wand-2" size={15} style={{ color: 'var(--brand-fg)' }} />
          </span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-strong)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{wizard.name}</div>
            <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 1 }}>Answers only from this Wizard’s {wizard.note_count != null ? `${wizard.note_count} ` : ''}pages</div>
          </div>
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
          {loading && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, color: 'var(--text-faint)', padding: 8 }}>loading history…</div>}
          {!loading && messages.length === 0 && (
            <div style={{ padding: '26px 10px', textAlign: 'center' }}>
              <div style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--text-strong)', marginBottom: 4 }}>Ask {wizard.name} anything.</div>
              <div style={{ fontSize: 12.5, color: 'var(--text-subtle)', lineHeight: 1.5 }}>It only knows the pages inside it — answers come with citations.</div>
            </div>
          )}
          {messages.map((m, i) => (
            m.role === 'user'
              ? <div key={i} style={{ display: 'flex', justifyContent: 'flex-end', margin: '10px 0' }}>
                  <div style={{ maxWidth: '85%', padding: '9px 13px', background: 'var(--obsidian-720)', borderRadius: '12px 12px 3px 12px', color: 'var(--text-strong)', fontSize: 13.5, lineHeight: 1.5, border: '1px solid var(--border-subtle)' }}>{m.text}</div>
                </div>
              : <div key={i} style={{ display: 'flex', gap: 10, margin: '12px 0' }}>
                  <span style={{ width: 24, height: 24, flexShrink: 0, marginTop: 2, borderRadius: 7, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'var(--brand-soft-bg)', border: '1px solid var(--brand-soft-border)' }}>
                    <WzIcon name="wand-2" size={12} style={{ color: 'var(--brand-fg)' }} />
                  </span>
                  <div style={{ flex: 1, minWidth: 0, fontSize: 13.5, lineHeight: 1.6, color: 'var(--text-body)', whiteSpace: 'pre-wrap' }}>{m.text}</div>
                </div>
          ))}
          {asking && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, color: 'var(--text-faint)', padding: '4px 0 4px 34px' }}>thinking…</div>}
        </div>
        <div style={{ padding: '10px 14px 14px', flexShrink: 0 }}>
          <div style={{ border: '1px solid var(--border-field)', borderRadius: 12, background: 'var(--surface-canvas)', padding: 10 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 6 }}>
              <button onClick={send} disabled={asking || !draft.trim()} aria-label="Send" style={{
              ...
              }}><WzIcon name="arrow-up" size={15} /></button>
            </div>
          </div>
        </div>
      </div>
...
function TodoWizardDrawer({ writeScope, readScopes, onClose }) {
  ...
  const act = async (id, kind) => {
    ...
    try {
      ...
      if (r && r.error) setErr(r.error);
      else setItems((xs) => xs.filter((t) => t.id !== id)); // leaves the current (status) view
    } catch (e) { setErr(String(e && e.message || e)); }
    setActing(null);
  };

  const Tab = ({ id, label }) => (
    <button onClick={() => setStatus(id)} style={{
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderBottom: '1px solid var(--divider)', flexShrink: 0 }}>
          <span style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'var(--brand-soft-bg)', border: '1px solid var(--brand-soft-border)' }}>
            <WzIcon name="list-checks" size={15} style={{ color: 'var(--brand-fg)' }} />
          </span>
          <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ padding: '12px 14px 0', flexShrink: 0 }}>
          <div style={{ border: '1px solid var(--border-field)', borderRadius: 12, background: 'var(--surface-canvas)', padding: 10 }}>
            <textarea value={thread} onChange={(e) => setThread(e.target.value)} rows={5}
              aria-label="Paste a thread" placeholder={'Paste a thread…\nFrom: Dana Ruiz <dana@…>\n- Marcus, send the plan by Friday.'}
              style={{ width: '100%', resize: 'vertical', border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-sans)', fontSize: 13, lineHeight: 1.5, color: 'var(--text-strong)' }} />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
              <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>Filed under <b style={{ color: 'var(--text-subtle)' }}>{writeScope}</b></span>
              <WzButton variant="amber-ghost" icon="sparkles" onClick={extract} disabled={busy || !thread.trim()}>
        ...
        <div style={{ display: 'flex', gap: 4, padding: '12px 14px 6px', flexShrink: 0 }}>
          ...
          <Tab id="dismissed" label="Dismissed" />
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 14px 16px' }}>
          {err && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--danger-fg, #e5484d)', padding: '6px 2px' }}>{err}</div>}
          ...
          {items.map((t) => (
            <div key={t.id} style={{ padding: 12, marginBottom: 10, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface-inset)' }}>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-strong)', lineHeight: 1.4 }}>{t.task}</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 6, fontSize: 11.5, color: 'var(--text-faint)' }}>
                {t.assignee && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><WzIcon name="user" size={12} />{t.assignee}</span>}
                {(t.due_text || t.due) && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><WzIcon name="calendar" size={12} />{t.due_text || t.due}</span>}
                {t.source && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><WzIcon name="quote" size={12} />{t.source}</span>}
              </div>
              {status === 'pending' && (
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  <WzButton variant="amber-ghost" icon="check" onClick={() => act(t.id, 'confirm')} disabled={acting === t.id} style={{ flex: 1 }}>
                    {acting === t.id ? '…' : 'Confirm'}
                  </WzButton>
                  <WzButton icon="x" onClick={() => act(t.id, 'dismiss')} disabled={acting === t.id} style={{ flex: 1 }}>Dismiss</WzButton>
                </div>
              )}
            </div>
          ))}
    ...
    <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', background: 'var(--surface-canvas)' }}>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '26px 30px 80px' }}>
        <button onClick={onBack}
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginBottom: 14, fontFamily: 'var(--font-sans)', fontSize: 12.5, color: hoverBack ? 'var(--text-strong)' : 'var(--text-subtle)' }}>
          <WzIcon name="arrow-left" size={14} />
          Back to {backLabel || 'your pages'}
        </button>
        <h1 style={{ fontSize: 23, fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--text-strong)', margin: '0 0 6px' }}>Wizards</h1>
        <p style={{ fontSize: 13, color: 'var(--text-subtle)', margin: '0 0 20px', lineHeight: 1.55, maxWidth: 560 }}>
        ...
        {/* Built-in people-work wizard — thread → action items. Always present. */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14, marginBottom: 14 }}>
          <WizardCard name="To-dos from a thread" icon="list-checks"
            meta="Paste an email chain → confirm/dismiss action items"
            onChat={null}
            extraAction={
              <WzButton variant="amber-ghost" icon="sparkles" onClick={() => setTodoOpen(true)} style={{ flex: 1 }}>
                Open
              </WzButton>
            } />
        </div>
      ...
      {todoOpen && <TodoWizardDrawer writeScope={todoWriteScope} readScopes={scopes} onClose={() => setTodoOpen(false)} />}
      {builderOpen && Builder && (
        <Builder scopes={scopes} onClose={() => setBuilderOpen(false)}
