...
const PageCard = React.memo(function PageCard({ note, section, sectionColor, snippet, fresh, placeMeta, place, owner, editor, onOpen, onChat }) {
  ...
  return (
    <div onClick={() => onOpen(note.id)} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={(e) => { e.stopPropagation(); onChat(note.id); }} style={{
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', background: 'var(--surface-canvas)' }}>
      <div style={{ maxWidth: 1060, margin: '0 auto', padding: '26px 30px 80px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        {notes.length > visible && <div ref={sentinelRef} style={{ height: 1 }} />}
      </div>
    </div>
  );
}
