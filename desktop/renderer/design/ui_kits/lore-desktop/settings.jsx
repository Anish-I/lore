...
function SettingsView({ settings, config, scopeOptions = [], onConfig, onOpenSetup }) {
  ...
  const stExportGithubPackage = async () => {
    ...
    } catch (e) { setGhPackageResult({ kind: 'error', error: String((e && e.message) || e) }); }
    finally { setGhPackageBusy(''); }
  };
  ...
  const stSetGithubAutoImport = async (enabled) => {
    ...
    if (r && r.ok) setGhPackage((p) => ({ ...(p || {}), autoImport: !!enabled }));
    else setGhPackageResult({ kind: 'error', error: (r && r.error) || 'Could not update automatic imports' });
  };
  ...
  return (
    <div style={stS.wrap}>
      <div style={stS.body}>
        ...
        <Section icon="user" title="Account">
          <div style={{ ...stS.row }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div title={`${tenantLabel}${scopeLabel ? ` · ${scopeLabel}` : ''}`}
                style={{ fontSize: 11.5, color: 'var(--text-faint)', marginTop: 2 }}>{accountKind}</div>
            </div>
        ...
        <Section icon="github" title="GitHub package">
          <Row
            label="Portable .lore package"
            hint="Commit .lore/package.json with a repository so teammates can pull its shared knowledge into Lore. Local account IDs, tags, and worklog stay in ignored .lore/manifest.json."
          >
            <StBadge tone={ghPackage && ghPackage.valid ? 'success' : ghPackage && ghPackage.exists ? 'warning' : 'neutral'} dot={!!(ghPackage && ghPackage.valid)}>
          ...
          <Row
            ...
            hint="Only notes with `share: github` in YAML frontmatter are included. Recognizable tokens are redacted, secret-dominated files are skipped, and unchanged exports do not rewrite the file. Review the diff before committing."
          >
            <StButton variant="secondary" size="sm" icon="upload" onClick={stExportGithubPackage} disabled={!!ghPackageBusy}>
              {ghPackageBusy === 'export' ? 'Packaging…' : ghPackage && ghPackage.exists ? 'Update package' : 'Create package'}
            </StButton>
          </Row>
          {ghPackage && ghPackage.valid && (
            <Row
              label="Import shared notes"
              hint={ghPackage.needsImport ? 'This package has knowledge not yet imported on this machine. Existing repository files remain the source of truth and are never overwritten.' : ghPackage.importedAt ? `Current · imported ${stAgo(ghPackage.importedAt)}` : 'Imports into Lore’s local index without creating or overwriting repository files.'}
            >
              <StButton variant={ghPackage.needsImport ? 'primary' : 'secondary'} size="sm" icon="download" onClick={stImportGithubPackage} disabled={!!ghPackageBusy}>
                {ghPackageBusy === 'import' ? 'Importing…' : ghPackage.needsImport ? 'Import update' : 'Import'}
              </StButton>
            </Row>
          )}
          {ghPackage && ghPackage.importedAt && (
            <Row label="Import updates on launch" hint="After you trust a package once, Lore can detect a changed content digest after git pull and import the revision automatically.">
              <StSwitch checked={!!ghPackage.autoImport} onChange={stSetGithubAutoImport} />
            </Row>
          )}
          {ghPackageResult && (
            <div style={{ padding: '10px 16px 12px', borderTop: '1px solid var(--divider)', fontFamily: 'var(--font-mono)', fontSize: 11.5, lineHeight: 1.6, color: ghPackageResult.kind === 'error' ? 'var(--clay-400)' : 'var(--text-muted)' }}>
