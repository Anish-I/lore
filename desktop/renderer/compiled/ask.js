...
function AnswerRuns({ runs, onCite }) {
  return runs.map((r, i) => {
    ...
    if (r.cite) return /*#__PURE__*/React.createElement(React.Fragment, { key: i }, r.x, /*#__PURE__*/React.createElement(CitationChip, { index: r.cite, note: "source", onClick: () => onCite && onCite(r.cite) }));
...
function Evidence({ rows }) {
  ...
  return (/*#__PURE__*/
    React.createElement("div", { style: { margin: '6px 0 4px 36px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', background: 'var(--surface-base)' } }, /*#__PURE__*/
    ...
    React.createElement(AkIcon, { name: "git-commit-horizontal", size: 13 }), /*#__PURE__*/
    React.createElement("span", null, "why retrieved \xB7 ", rows.length, " chunks"), /*#__PURE__*/
    React.createElement(AkIcon, { name: "chevron-down", size: 13, style: { marginLeft: 'auto', transform: open ? 'none' : 'rotate(-90deg)', transition: 'transform var(--dur-fast) var(--ease-out)' } })
    ),
    open && /*#__PURE__*/
...
function CitationSources({ citations, onCiteScope }) {
  ...
  return (/*#__PURE__*/
    ...
    visible.map((c) => /*#__PURE__*/
    React.createElement("span", { key: c.note_id,
      ...
      style: { display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 8px', border: '1px solid var(--border)', borderRadius: 'var(--radius-full)', background: 'var(--surface-inset)', fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--text-muted)', maxWidth: 220 } }, /*#__PURE__*/
    React.createElement("span", { style: { width: 6, height: 6, borderRadius: '50%', background: AkCiteDot(c.scope), flexShrink: 0 } }), /*#__PURE__*/
    React.createElement("span", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, c.title || c.note_id), /*#__PURE__*/
    React.createElement("span", { style: { color: 'var(--text-faint)', flexShrink: 0 } }, "\xB7 ", AkCiteScopeLabel(c.scope))
    )
    ),
    hidden > 0 && /*#__PURE__*/
    React.createElement("button", { onClick: () => setExpanded(true), style: { padding: '2px 10px', border: '1px dashed var(--border)', borderRadius: 'var(--radius-full)', background: 'transparent', color: 'var(--text-faint)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 10.5 } }, "+", hidden, " more"),

    expanded && unique.length > 4 && /*#__PURE__*/
    React.createElement("button", { onClick: () => setExpanded(false), style: { padding: '2px 10px', border: 'none', background: 'transparent', color: 'var(--text-faint)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 10.5 } }, "less")
...
function SourceToggle({ value, onChange, options }) {
  ...
  return (/*#__PURE__*/
    React.createElement("div", { style: { display: 'flex', minWidth: 0, maxWidth: 190, background: 'var(--surface-inset)', border: '1px solid var(--border)', borderRadius: 'var(--radius-full)', padding: 2, gap: 2, overflow: 'hidden' } },
    opts.map((o) => /*#__PURE__*/
    React.createElement("button", { key: o.id, onClick: () => onChange(o.id), title: `Ask from ${o.label}`, style: {
    ...
    React.createElement(AkIcon, { name: o.icon, size: 11 }), /*#__PURE__*/React.createElement("span", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, o.label)
    )
    )
    ));
...
function AskPanel({ messages, asking, suggestions, onSend, onClose, source, onSource, sourceOptions, identityReady, onSetup,
  ...
  return (/*#__PURE__*/
    ...
    React.createElement(AkIcon, { name: "sparkles", size: 14, style: { color: 'var(--brand-fg)' } })
    ), /*#__PURE__*/
    React.createElement("span", { style: { flex: 1, fontSize: 14, fontWeight: 600, color: 'var(--text-strong)' } }, "Ask Lore"),


    sourceOptions && sourceOptions.length > 2 && /*#__PURE__*/
    ...
    React.createElement("div", { style: { padding: '24px 6px' } }, /*#__PURE__*/
    React.createElement("img", { src: "design/assets/sprites/lore-familiar.png", alt: "", "aria-hidden": "true",
      style: { display: 'block', width: 132, height: 132, margin: '0 auto 10px', objectFit: 'contain', filter: 'drop-shadow(0 6px 16px rgba(0,0,0,0.28))', pointerEvents: 'none', userSelect: 'none' } }), /*#__PURE__*/
    React.createElement("p", { style: { fontFamily: 'var(--font-serif)', fontSize: 17, color: 'var(--text-body)', margin: '0 0 4px', textAlign: 'center' } }, "Ask across your libraries."), /*#__PURE__*/
    React.createElement("p", { style: { fontSize: 13, color: 'var(--text-subtle)', margin: '0 0 16px', lineHeight: 1.5, textAlign: 'center' } }, "Answers are drawn only from notes you can see, and every claim is cited."),
    ...
    React.createElement(AkIcon, { name: "corner-down-right", size: 14, style: { color: 'var(--text-faint)' } }), s
    )
    ),
    suggestions.length > 0 && /*#__PURE__*/
    React.createElement("div", { style: { fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-faint)', textAlign: 'center', marginTop: 2 } }, "these learn from what you ask")
    ...
    React.createElement(AskMessage, { key: i, role: "user" }, m.text) : /*#__PURE__*/

    React.createElement("div", { key: i }, /*#__PURE__*/
    React.createElement(AskMessage, { role: "answer", sources: m.streaming ? undefined : m.sources, scopes: m.streaming ? undefined : m.scopes, streaming: m.streaming },
    ...
    showCites && !m.streaming && /*#__PURE__*/React.createElement(CitationSources, { citations: m.citations, onCiteScope: onCiteScope }),
    showCites && !m.streaming && m.evidence && /*#__PURE__*/React.createElement(Evidence, { rows: m.evidence })
    )
    ...
    React.createElement("label", { style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--text-body)', cursor: 'pointer' } }, /*#__PURE__*/
    React.createElement("input", { type: "checkbox", checked: showCites, onChange: (e) => setShowCites(e.target.checked) }), "Cite sources"
