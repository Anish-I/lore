...
function BackBar({ label, onBack }) {
  ...
  return (/*#__PURE__*/
    React.createElement("div", { style: { flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--surface-panel)' } }, /*#__PURE__*/
    React.createElement("button", { onClick: onBack,
      onMouseEnter: () => setHover(true), onMouseLeave: () => setHover(false),
      style: { display: 'inline-flex', alignItems: 'center', gap: 6, height: 30, padding: '0 12px', borderRadius: 8, border: '1px solid var(--border)', cursor: 'pointer', background: hover ? 'var(--surface-hover)' : 'transparent', color: hover ? 'var(--text-strong)' : 'var(--text-body)', fontFamily: 'var(--font-sans)', fontSize: 12.5, fontWeight: 500 } }, /*#__PURE__*/
    React.createElement(D.Icon, { name: "arrow-left", size: 14 }), "Back to ",
    label || 'your pages'
    )
    ));
...
function NotesRail({ section, notes, activeId, chatActive, noteMeta, onOpen, onChat }) {
  return (/*#__PURE__*/
    ...
    React.createElement("div", { style: { padding: '2px 10px 8px', fontSize: 12.5, fontWeight: 600, color: 'var(--text-strong)', display: 'flex', alignItems: 'baseline', gap: 6 } }, /*#__PURE__*/
    React.createElement("span", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, section), /*#__PURE__*/
    React.createElement("span", { style: { fontSize: 10.5, color: 'var(--text-faint)', fontFamily: 'var(--font-mono)' } }, notes.length)
    ), /*#__PURE__*/
    React.createElement("button", { onClick: onChat, style: {
    ...
    (() => {const I = (window.VaultDesignSystem_ffbf58 || {}).Icon;return I ? /*#__PURE__*/React.createElement(I, { name: "sparkles", size: 13, style: { color: 'var(--brand-fg)', flexShrink: 0 } }) : null;})(), /*#__PURE__*/
    React.createElement("span", { style: { fontWeight: 600 } }, "Section chat")
    ), /*#__PURE__*/
    React.createElement("div", { style: { flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 1 } },
    notes.map((n) => {
      const active = n.id === activeId;
      const snippet = (noteMeta[n.id] || {}).snippet || '';
      return (/*#__PURE__*/
        React.createElement("button", { key: n.id, onClick: () => onOpen(n.id), title: n.name, style: {
        ...
        React.createElement("div", { style: { fontSize: 12.5, fontWeight: active ? 600 : 500, color: active ? 'var(--text-strong)' : 'var(--text-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, n.name),
        snippet && /*#__PURE__*/React.createElement("div", { style: { fontSize: 11, color: 'var(--text-faint)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 1 } }, snippet)
        ));

    }),
    !notes.length && /*#__PURE__*/
    React.createElement("div", { style: { padding: '6px 10px', fontSize: 11.5, color: 'var(--text-faint)', lineHeight: 1.5 } }, "No pages here yet.")
...
function App() {
  ...
  const createTeam = React.useCallback(async (name) => {
    ...
    try {
      ...
      if (!user) {
        ...
        if (!res || !res.ok) {setTeamError(res && res.body && res.body.detail || 'Could not create the team.');} else
        {
  ...
  const askPanel = /*#__PURE__*/React.createElement(AskPanel, { messages: messages, asking: asking, suggestions: askSuggestions, onSend: ask, onClose: () => {setAskOpen(false);setAskCtx(null);}, source: askSource, onSource: setAskSource, sourceOptions: askSourceOptions, identityReady: identityReady, onSetup: () => {setView('settings');setShowOnboarding(true);},
  ...
  return (/*#__PURE__*/
    ...
    React.createElement(PlacesBar, { place: place, onPlace: setPlace, counts: placeCounts,
    ...
    React.createElement(Ribbon, { place: place, askOpen: askOpen, mapOpen: mapOpen, wizardsOpen: view === 'wizards',
      ...
      onRefresh: async () => {
        ...
        catch {flash('Refresh failed — is the backend running?');} finally
        {setRefreshing(false);}
      },
    ...
    !treeData ? /*#__PURE__*/
    React.createElement(EmptyEditor, null) : /*#__PURE__*/

    React.createElement(React.Fragment, null,
    !(place === 'team' && !inTeam && placeCounts.team === 0) && /*#__PURE__*/
    React.createElement(SectionRail, { sections: railSections, allCount: placeCounts[place],
    ...
    kbFilter.length > 0 && !activeBucket && /*#__PURE__*/
    React.createElement(NotesRail, { section: kbFilter.length === 1 ? kbFilter[0] : `${kbFilter.length} sections`,
    ...
    React.createElement(Editor, { bucket: activeBucket, tabs: tabs, activeId: activeId, onTab: onTab, onCloseTab: closeTab, onCloseOthers: closeOtherTabs, hideTabs: true, onOpen: () => setAskOpen(true) }) :
    editorNote ? /*#__PURE__*/
    React.createElement(PageView, { note: editorNote, place: place, mode: mode,
    ...
    React.createElement(SectionRail, { sections: railSections, allCount: placeCounts[place],
      ...
      onWizards: () => {setMapOpen(false);setView('wizards');},
      onPeople: () => {setMapOpen(false);setView('people');},
      onMoveSection: moveSection, sectionMoveBusy: sectionMoveBusy }), /*#__PURE__*/
    ...
    React.createElement("div", { style: { flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 } }, /*#__PURE__*/
    React.createElement(BackBar, { label: (window.LorePlaceMeta[place] || {}).label, onBack: () => setView('workspace') }), /*#__PURE__*/
    React.createElement(HooksView, { scopeOptions: scopeOptions, identityReady: identityReady, tenant: tenant, scope: persona.scopes && persona.scopes[0], onOpenSetup: () => setShowOnboarding(true) })
    )
