...
function App() {
  ...
  const ask = async (q) => {
    ...
    let trace;
    try { trace = await window.lore.ask(q, scopes, tenant); }
    catch (e) {
  ...
  const askPanel = <AskPanel messages={messages} asking={asking} suggestions={suggestions} onSend={ask} onClose={() => setAskOpen(false)} source={askSource} onSource={setAskSource} />;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: 'var(--surface-sunken)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, position: 'relative' }}>
          ...
          <React.Fragment>
            ...
            {activeBucket
              ...
              : (editorNote
                ...
                : <EmptyEditor />)}
            {askOpen ? askPanel : (editorNote && <ContextPane note={editorNote} onAsk={() => setAskOpen(true)} />)}
          </React.Fragment>
        )}
        ...
        {view === 'graph' && (<React.Fragment><GraphView graph={M.graph} onOpen={() => setView('workspace')} />{askOpen && askPanel}</React.Fragment>)}
        {view === 'buckets' && (<React.Fragment><BucketsView buckets={M.buckets} onAsk={() => setAskOpen(true)} onOpen={openBucket} />{askOpen && askPanel}</React.Fragment>)}
