  ...
  const r = await win.evaluate(async ({ tenant, scopes }) => {
    ...
    try {
      if (tenant && scopes.length) {
        ...
        out.graphNodes = g.nodes.length; out.graphEdges = g.edges.length;
      } else out.graphSkipped = 'tenant/scopes not configured';
    } catch (e) { out.graphErr = String(e); }
    // AI WITH TEAM SCOPE — ask across private+team+enterprise
    try {
      if (tenant && scopes.length) {
        const t = await window.lore.ask('What is the Kalshi trading bot architecture?', scopes, tenant);
        out.askEngine = t.engine; out.askChunks = (t.final || []).length; out.askAnswerChars = (t.answer || '').length;
      } else out.askSkipped = 'tenant/scopes not configured';
    } catch (e) { out.askErr = String(e); }
