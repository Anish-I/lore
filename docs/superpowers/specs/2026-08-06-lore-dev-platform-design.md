...
| Teams reach | `lore host` = engine server mode + secure tunnel; join code = tunnel URL + scoped invite token |
| Build strategy | Big-bang platform: all subsystems designed now, built as one coordinated push (internally dependency-ordered) |
| Tenancy surface | **No organizations anywhere.** Hierarchy is `you → teams → projects`. Engine `orgs` tables stay dormant. |
...
branch: dev
commits: [abc1234]         # commits made during the session — the join key to code
files_touched: [core/auth.py, tests/test_auth.py]
status: shipped            # shipped | abandoned | in-progress
---
# Fix auth token race
...
managed source of truth for session/decision records. Engine stores (SQLite
notes/chunks/edges + Qdrant vectors) are **derived and rebuildable** from
`.lore/` + personal notes. Expand-context reads the repo itself as truth.
Deletes propagate: folding or deleting a record tombstones it in the engine
(`folded_paths`) and purges vectors downstream on next sync.
...
being handed false certainty. (The board's NLI fact-check seat is cut from v1 —
see Cuts.)
...
*what the AI did vs what survived review and merge*: session `status`
(shipped/abandoned) + the diff between session commits and what lands in the
default branch. v1 **captures** this grade in session frontmatter and feedback
events; weight-nudging (bounded, capped, per the board) is enabled only after
...
(:8099, VBS launcher + crash-retry), file watcher re-indexing `.lore/` on save,
nightly `LoreUpkeep` (fold, classify, decay), nightly eval gate, staleness
watchdog (db mtime + port probe — and **assert listening PID == launched PID**,
the zombie lesson).

### Team mode — org-free

- `lore host` — flips the local engine to serve selected projects: binds the
  API for remote access, opens a secure tunnel (Tailscale Funnel preferred,
  Cloudflare quick tunnel fallback), mints a **join code** = tunnel URL + scoped
  invite token (engine's existing invite tables; no org layer touched). One
  public port, TLS at the tunnel edge — the board's Caddy principle.
- `lore join <code>` — registers the remote brain; those projects appear in the
  teammate's recall lane, ACL-filtered in-query. Live while the host PC is on.
