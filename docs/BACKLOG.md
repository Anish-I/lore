...
- **M6 B2B connectors + hosted** — deliberately NOT stubbed: blocked on (1) the 90-day plan's own gate — lock the warm-door insurance/regulated vertical first, (2) Google/Slack OAuth app credentials to build+test against, (3) server-mode agent/principal authn (item I3 below) before anything writes cross-network. **Update 2026-07-18:** enterprise identity is now built — Okta SSO login (`POST /auth/okta`, `core/lore/okta.py`) derives team scopes from the token's `groups` claim via `OKTA_GROUP_SCOPE_MAP`, so blocker (3)'s multi-user gate exists behind `LORE_SERVER_MODE=1`. The desktop Okta loopback sign-in is now built too (`desktop/lib/okta-oauth.js` → `auth:login-okta` → `/auth/okta`; "Continue with Okta SSO" in the auth modal; env-first config or gitignored `secrets/okta_client.json`). **The connector track is now opened:** local **mailbox → to-dos** (`POST /connectors/mailbox/sync`, `.eml` folders) and **Slack workspace-export → to-dos** (`POST /connectors/slack/sync`) connectors ingest into the to-dos wizard idempotently (`connector_seen` watermark), scope-governed like the wizard, both surfaced in the desktop to-dos drawer. Two source parsers over one shared substrate (`core/lore/connectors.py`) — proving the abstraction. Remaining for hosted: the connector **OAuth creds (Google/Slack)** in (2), which turn the same pipeline into a live Gmail/Slack **API** pull (only the fetch step is new). **Rotate the Okta client secret — it was shared over chat.**
...
- **XLSX ingestion — added 2026-07-17** (ghandour-branch). `extract.py` now reads `.xlsx` via openpyxl (read-only stream, cell-cap for bomb safety); rows render pipe-delimited so header↔value structure survives chunking. Found while building the synthetic corpus (`synth/`) — spreadsheets are core people-work for the enterprise direction. Test in `test_extract.py`.
- **PPTX ingestion — still unsupported.** No `python-pptx` on the dev box; the synthetic corpus renders "presentations" as PDF slide-decks for now. Add real `.pptx` extraction (slide text + speaker notes) if decks become first-class content.
