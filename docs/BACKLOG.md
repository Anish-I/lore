...
- **M6 B2B connectors + hosted** — deliberately NOT stubbed: blocked on (1) the 90-day plan's own gate — lock the warm-door insurance/regulated vertical first, (2) Google/Slack OAuth app credentials to build+test against, (3) server-mode agent/principal authn (item I3 below) before anything writes cross-network. **Update 2026-07-18:** enterprise identity is now built — Okta SSO login (`POST /auth/okta`, `core/lore/okta.py`) derives team scopes from the token's `groups` claim via `OKTA_GROUP_SCOPE_MAP`, so blocker (3)'s multi-user gate exists behind `LORE_SERVER_MODE=1`. Remaining for hosted: the connector OAuth creds (Google/Slack) in (2) and the desktop Okta loopback sign-in. **Rotate the Okta client secret — it was shared over chat.**
