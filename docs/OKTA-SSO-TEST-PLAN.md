...
- [ ] **1.3 Server RS256 verification, end-to-end (real crypto, no monkeypatch)**
  ``bash cd core VAULT_FAKE=1 python -m pytest tests/test_okta_verify_e2e.py -p no:asyncio -q ``
...
- [ ] **1.4 Desktop loopback, end-to-end (real flow vs. mock Okta over HTTPS)**
  ``bash cd desktop npm run test:okta ``
...
- [ ] **1.5 Full Electron boot smoke** — launches the REAL app with Playwright and checks the Okta
  button wiring + the "not configured" path (**Parts 3.2 / 5.1**). Runs as part of `npm test` (it's
  the slow one — it boots the whole app). Run it alone with:
  ``bash cd desktop npm run test:e2e:electron ``
  **✅ Pass =** `1 passed`. On a genuinely display-less box it skips cleanly (never an error); it does
  **not** skip just because a flag is missing.
  > Gotcha: if your shell has `ELECTRON_RUN_AS_NODE=1` set (common in CI/agent shells), Electron runs
