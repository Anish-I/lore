...
def test_flag_modes(monkeypatch):
    # forced on/off always win, regardless of whether the engine is installed
    monkeypatch.setenv("LORE_OCR_FALLBACK", "1")
    assert ocr.mode() == "on" and ocr.enabled() is True
    monkeypatch.setenv("LORE_OCR_FALLBACK", "0")
    assert ocr.mode() == "off" and ocr.enabled() is False

    # unset (and explicit 'auto') = auto: enabled iff the optional engine is installed
    monkeypatch.delenv("LORE_OCR_FALLBACK", raising=False)
    assert ocr.mode() == "auto"
    monkeypatch.setattr(ocr, "available", lambda: True)
    assert ocr.enabled() is True
    monkeypatch.setattr(ocr, "available", lambda: False)
    assert ocr.enabled() is False
    monkeypatch.setenv("LORE_OCR_FALLBACK", "auto")
    assert ocr.mode() == "auto" and ocr.enabled() is False


def test_auto_mode_degrades_to_text_path(monkeypatch, tmp_path):
    """Auto + missing/broken engine must never fail a PDF the historical
    text-only path can read; forced-on keeps raising (operator asked for OCR)."""
    fitz = pytest.importorskip("fitz")
    pdf = tmp_path / "native.pdf"
    doc = fitz.open()
    page = doc.new_page()
    page.insert_text((72, 100), "The mill rate holds at thirty four point two next year.")
    doc.save(str(pdf))

    def boom(path):
        raise ocr.OCRUnavailable("engine broken")
    monkeypatch.setattr(ocr, "extract_pdf_routed", boom)
    monkeypatch.setattr(ocr, "available", lambda: True)   # auto believes it's installed

    monkeypatch.delenv("LORE_OCR_FALLBACK", raising=False)
    result = extract.extract_document(str(pdf))
    assert result is not None and "mill rate" in result.text.lower()

    monkeypatch.setenv("LORE_OCR_FALLBACK", "1")
    with pytest.raises(ocr.OCRUnavailable):
        extract.extract_document(str(pdf))
