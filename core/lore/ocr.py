...
'1' forces on (missing engine raises), '0' forces off, unset/'auto' enables
the router whenever the optional RapidOCR dependency is installed — so a
scanned PDF is recovered out of the box on any install that has the models,
while a bare install keeps the historical text-only path with no new errors.
...
def mode() -> str:
    """'on' | 'off' | 'auto' from LORE_OCR_FALLBACK ('1' / '0' / unset-or-'auto')."""
    v = os.environ.get("LORE_OCR_FALLBACK", "auto").strip().lower()
    if v in ("1", "true", "on"):
        return "on"
    if v in ("0", "false", "off"):
        return "off"
...
def available() -> bool:
    """The optional RapidOCR engine is installed. Metadata check only — never
    loads the ONNX models, so it is safe to call on every extraction."""
